...
def produce_superellipsoids_array(dir_save, n_sampled):
    a = [1, 1, 1]
    ...
    for i in range(n_choice + 1):
        for j in range(n_choice + 1):
            ...
            path_save = os.path.join(dir_save, "superellipsoids", "el_%d_%d.pts" % (i, j))
            ...
            y_all.append(y)
            z_all.append(z)
    save_objs(os.path.join(dir_save, "all_superellipsoids.obj"), x_all, y_all, z_all)
...
if __name__ == '__main__':
    # dir_save = "res"
    # n_sampled = 20 # But somehow produce n_sampled + 1 vertices
    # produce_superellipsoid_array(dir_save, n_sampled)
    # produce_supertoroids_array(dir_save, n_sampled)

    dir_save = "res_32x32"
    n_sampled = 31 # But somehow produce n_sampled + 1 vertices
    produce_superellipsoids_array(dir_save, n_sampled)
    produce_supertoroids_array(dir_save, n_sampled)
